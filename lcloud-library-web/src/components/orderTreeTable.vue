  ...
  export default{
  	...
  	methods:{
  		iconShow(index, record) {
  			if(index === 0 && record.batchs && record.batchs.length > 0){
  				return "visible"
  			}else{
  				return "hidden"
  			}
    	...
    	//表格行是否显示
		  ...
		  Array.from(data).forEach(function(record) {
		    ...
		    tmp.push(record)
		    if (record.batchs && record.batchs.length > 0) {
		      const batchs =_this.treeToArray(record.batchs, expandAll, record, _level)
		      tmp = tmp.concat(batchs)
		    }
		  })
		  return tmp
		}
