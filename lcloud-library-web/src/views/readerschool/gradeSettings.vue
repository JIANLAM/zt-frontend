<template>
   <div id="gradeSettings">
      ...
      <el-dialog :visible.sync="visible"  :title="titleName" :before-close='closeDialog' class="collectionParameters">
        <el-form :model="levelForm" :rules="rules" ref="ruleForm">
            <el-form-item label="学段" :label-width="formLabelWidth" prop="type">
              <el-select v-model="levelForm.type" @change='typeChange' style="width: 100%">
            ...
            <el-form-item label="年级名称" :label-width="formLabelWidth" prop="name">
              <el-select v-model="levelForm.name"  @change='nameChange' style="width: 100%">
            ...
            <el-form-item label="年级代码" :label-width="formLabelWidth" prop="code">
              <el-input v-model="levelForm.code" auto-complete="off" ></el-input>
            </el-form-item>
            <el-form-item label="年级级别" :label-width="formLabelWidth" prop="level">
              <el-input :disabled="true" v-model="levelForm.level" auto-complete="off" ></el-input>
            </el-form-item>
            <el-form-item label="证类型" :label-width="formLabelWidth" prop="readerCardId">
              <el-select v-model="levelForm.readerCardId" placeholder="请选择" >
                <el-option v-for="item in readerCardData" :key="item.id" :label="item.label" :value="item.id"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="备注" :label-width="formLabelWidth">
              <el-input type="textarea" v-model="levelForm.remark" auto-complete="off" ></el-input>
            </el-form-item>
...
export default {
  data() {
    ...
    return {
      ...
      titleName: "新增年级信息",
      rules: {
        name: [{ required: true, message: '年级名称不能为空', trigger: "change" }],
        code: [{ required: true, message: '年级代码不能为空', trigger: "blur" },
        {validator: checkNumber, trigger: "blur"}],
        type: [{ required: true, message: '学段不能为空', trigger: "change" }],
        readerCardId: [{ required: true, message: '所属部门不能为空', trigger: "change" }]
      },
      form: {
        pageNum: 1,
        pageSize: 10
