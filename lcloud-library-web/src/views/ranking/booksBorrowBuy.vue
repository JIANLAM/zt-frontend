<template>
  <div id="ranking list">
   ...
   <el-table
      ...
      <el-table-column  prop="recount"  label="借购次数">  </el-table-column>
    </el-table>
		<el-row>
  			<el-col :span="24">
		  		<el-pagination
...
<script>
import { toplendbuybook } from "@/request/ranking";
...
export default {
  ...
  created() {
    this.getPowerArr();
    this.getToplendbuybook(JSON.stringify(this.form));
  },
  methods: {
    //得到权限按钮数组
    getPowerArr(){
    ...
    getToplendbuybook(obj) {
      this.listLoading = true;
      toplendbuybook(obj).then(data => {
    ...
    //导出
    exportList(){
      this.$confirm('是否要导出这'+this.data.length+'条图书借购排行榜信息?', '提示', {
        ...
        toplendbuybook(this.exportBlist).then(data=>{
          if(data.data.code==0){
