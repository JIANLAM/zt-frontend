  ...
  <div id="ranking list">
      ...
      <el-form-item>
        ...
        <el-table
            ...
            <el-table-column  prop="recount" label="荐购图书总数"></el-table-column>
       </el-table>
...
<script>
import { toprecommbuyreader } from "@/request/ranking";
export default {
	data(){
  		return {
  			listLoading:false,
		...
		created() {
      this.getPowerArr();
			this.getToprecommbuyreader(JSON.stringify(this.form))
		},
		methods: {
      //得到权限按钮数组
      getPowerArr(){
    ...
    getToprecommbuyreader(obj) {
      this.listLoading = true;
      toprecommbuyreader(obj).then(data => {
		...
		//导出
    exportList(){
      this.$confirm('是否要导出这'+this.data.length+'条读者荐购排行榜信息?', '提示', {
        ...
        toprecommbuyreader(this.exportBlist).then(data=>{
          if(data.data.code==0){
