<template>
  ...
  <el-table
     :data="data" border  style="width: 100%" v-loading='listLoading'>
      ...
      <el-table-column  prop="recount" label="借阅次数"></el-table-column>
    </el-table>
		<el-row>
  			<el-col :span="24">
		  		<el-pagination
...
<script>
import { toplendbook } from "@/request/ranking";
...
export default {
  ...
  created() {
    this.getPowerArr();
    this.getToplendbook(JSON.stringify(this.form));
  },
  methods: {
    //得到权限按钮数组
    getPowerArr(){
    ...
    //导出
    exportList(){
      this.$confirm('是否要导出这'+this.data.length+'条图书借阅排行榜信息?', '提示', {
        ...
        toplendbook(this.exportBlist).then(data=>{
          if(data.data.code==0){
