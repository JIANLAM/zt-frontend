<template>
  ...
  <el-table
     :data="data" border  style="width: 100%">
      ...
      <el-table-column  prop="recount" label="荐购次数" ></el-table-column>
  </el-table>
	<el-row>
  			<el-col :span="24">
		  		<el-pagination
...
<script>
import { toprecommbuybook } from "@/request/ranking";
...
export default {
  ...
  created() {
    this.getPowerArr();
    this.getToprecommbuybook(JSON.stringify(this.form));
  },
  methods: {
    //得到权限按钮数组
    getPowerArr(){
    ...
    getToprecommbuybook(obj) {
      this.listLoading = true;
      toprecommbuybook(obj).then(data => {
    ...
    //导出
    exportList(){
      this.$confirm('是否要导出这'+this.data.length+'条图书荐购排行榜信息?', '提示', {
        ...
        toprecommbuybook(this.exportBlist).then(data=>{
          if(data.data.code==0){
