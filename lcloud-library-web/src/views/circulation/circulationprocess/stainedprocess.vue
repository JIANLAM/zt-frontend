    ...
    <article class="book_lend_info">
      ...
      <el-table
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="showProcess(scope.row)">污损处理</el-button>
          </template>
        </el-table-column>
      </el-table>
    </article>
<!--污损处理弹框-->
    <el-dialog :visible.sync="processState" title="污损处理" width='400px' >
...
<script>
  import { lendBook, lendReader, readerlendbookInfo, stained } from "@/request/circulation";
  import moment from 'moment';
  export default {
    data(){
      // var forfeit = (rule, value, callback) => {
      //   if(!value) {
      //       return callback(new Error('罚款金额不能为空'));
      //   }
      //   if (!Number.isInteger(value)) {
      //       callback(new Error('罚款金额必须为整数数字值'));
      //     }
      //   else {
      //     callback();
      //   }
      // };
      // var payMethod = (rule, value, callback) => {
      //   if(!value) {
      ...
      //   if(!value) {
      //       return callback(new Error('请选择是否缴纳'));
      //   }else {
      //     callback();
      //   }
      // };
    ...
    methods:{
      ...
      stainedBtn() {
        ...
        this.$refs.dynamicValidateForm.validate(valid => {
          if(valid) {
            stained(obj).then(data=>{
              if(data.data.code == 0) {
                this.processState= false;
                this.message = '污损处理提交成功';
                this.stainedData = {};
