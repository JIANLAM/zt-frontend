<template>
    <div>
       ...
       <el-row>
        ...
        <!-- 添加 -->
        <el-dialog :visible.sync="visible" title="借阅规则"  class="collectionParameters borrowingrules">
            <el-form ref="addForm" :model="addForm" label-width="110px" :rules="rules" class="clearfix">
                <el-form-item label="规则类别" prop="ruleType">
                  <el-select v-model="addForm.ruleType"  placeholder="请选择" :style="{width:'100%'}">
                    <el-option label="馆内借阅" :value="label0"></el-option>
                    <el-option label="馆际借阅" :value="label1"></el-option>
                  </el-select>
                </el-form-item>
                ...
                <el-form-item label="可借文献类型" prop="literatureType">
                        <el-select v-model="literatureType"  placeholder="请选择" :style="{width:'100%'}" multiple >
                            <el-option :label="item.label" :value="item.id.toString()" v-for="item in circulationList" :key="item.value"></el-option>
                        </el-select>
                </el-form-item>
                <el-form-item label="是否启用" prop="status">
                        <el-select v-model="addForm.status"  placeholder="请选择" :style="{width:'100%'}">
                            <el-option label="是" :value="label1"></el-option>
                            <el-option label="否" :value="label0"></el-option>
                        </el-select>
                </el-form-item>
                <el-form-item class="addSure addSure1"  label-width="70px">
    ...
    // 添加
        add(row){
            ...
            if(row=="add"){
                ...
                if(row.literatureType!=""){
                if(typeof  row.literatureType!="object"){ this.literatureType=(row.literatureType==null?"":row.literatureType.split(","));}}
                this.addForm={
                  id: row.id,
                  ruleName: row.ruleName,
                  lendQty:row.lendQty ,
                  lendDays:row.lendDays ,
                  renewQty: row.renewQty,
                  ...
                  credit:row.credit ,
                  literatureTyperow:row.literatureTyperow ,
        ...
        submitForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                  this.addForm.literatureType=this.literatureType.join(",")
