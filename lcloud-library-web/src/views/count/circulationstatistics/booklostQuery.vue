  ...
  <div id="currentquery">
    <el-form :inline="true">
      ...
      <span v-for="(item,index) in power" :key="index">
        ...
        <el-button v-if="item.name.trim()=='导出'" type="warning" style="margin-bottom: 20px" icon="iconfont icon-daoru" @click="exportData">导出</el-button>
      </span>
    </el-form>
    <el-table
...
<script>
  import {bookLostSearch,SearCirculationTypeList,bookLostSearchExport} from '@/request/dataSearchCurrent'
  export default {
    data(){
      return {
        pageNum:1,//页码数
    ...
    created(){
      ...
      this.getBookLostInfo(this.getReaderInfoParams());
    },
    methods:{
      //得到权限按钮数组
      getPowerArr(){
      ...
      getBookLostInfo(params){
        bookLostSearch(params).then(res =>{
          if (res.data.code == 0){
      ...
      searchData(){
        ...
        this.getBookLostInfo(this.getSearchReaderInfoParams());
      },
      //导出
      exportData(){
        ...
        this.$confirm('是否要导出这'+this.totalPages+'条读者数据?', '提示', {
          ...
          bookLostSearchExport(params).then(res=>{
            if(res.data.code==0){
