  ...
  <div id="currentquery">
    <el-form :inline="true" :model="form">
      ...
      <span v-for="(item,index) in power" :key="index">
        ...
        <el-button v-if="item.name.trim()=='导出'" type="warning" style="margin-bottom: 20px" icon="iconfont icon-daoru" :disabled="tableDataA.length==0" @click="exportData">导出</el-button>
      </span>
    </el-form>
    <el-table
    ...
    <el-dialog :visible.sync="dialogImportVisible" title="图书借阅详情" :width="'1200px'" :center="true">
      <el-table
        ...
        <el-table-column  label="操作日期" prop="lendTime"></el-table-column>
        <el-table-column  label="操作类型" prop="lendStatus">
          <template slot-scope="scope">
...
<script>
  import {bookLendSearch,bookLendHistoryDetailSearch,bookLendHistoryDetailExport} from '@/request/dataSearchCurrent'
  export default {
    data(){
      return {
        pageNum:1,//页码数
    ...
    methods:{
      ...
      exportData(){
        ...
        this.$confirm('是否要导出这'+this.totalPagesA+'条图书借阅详情的数据?', '提示', {
          ...
          bookLendHistoryDetailExport(params).then(res=>{
            if(res.data.code==0){
