        ...
        <el-form ref="form" :model="form" label-width="120px">
            ...
            <el-form-item label="图书流通类型">
              ...
              <el-form-item label="操作类型">
                <el-select v-model="form.operationType" style="width: 420px;" >
                    <el-option label="全部" value=""></el-option>
                    <el-option label="入藏" value="入藏"></el-option>
                    <el-option label="变更" value="变更"></el-option>
                    <el-option label="剔除" value="剔除"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="操作人员">
                <el-select v-model="form.userId" style="width: 420px;" >
                    <el-option :label="item.login_name" :value="item.id" v-for="item in operationStaff" :key="item.id"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="条码号范围">
                    <el-input v-model="form.barcodeStart" style="width: 210px;">-</el-input>
                    <el-input v-model="form.barcodeEnd" style="width: 210px;"></el-input>
            </el-form-item>
...
import { collManageCount,exportCollection,operationStaff} from '@/request/count'
export default {
    data() {
      return {
        loading:true,
     ...
     methods: {
         ...
         searchData(){
           ...
           this.form.columnType=this.columnType;
            this.detail=true;
            this.loading=true;
            collManageCount(JSON.stringify(this.form)).then(data=>{
                if(data.data.code==0){
                    this.data=[]
                    this.loading=false;
        ...
        reset(){
            ...
            this.form.userId=this.operationStaff[0].id;
    ...
    created(){
        ...
        collAddressList().then(data=>{
         ...
         // 馆藏处理统计获取操作人员接口
        operationStaff().then(data=>{
            if(data.data.code==0){
                this.operationStaff=data.data.userMap;
                this.operationStaff.unshift({"login_name":"全部","id":""})
            }else{
                this.$message({
                type: 'error',
