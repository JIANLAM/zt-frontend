	...
	<div class="container">
      ...
      <span v-if="searchHidden">
        ...
        <el-form-item label="图书状态">
				<el-select v-model="searchForm.prebookStatus" placeholder="请选择" :style="{width:'120px'}">
				  <el-option label="所有"   value=""></el-option>
				  <el-option label="未入藏" value="1"></el-option>
					<el-option label="已入藏" value="2"></el-option>
				</el-select>
			</el-form-item>
      </span>
			<el-form-item label="">
        <span v-for="(item,index) in power" :key="index">
 		...
 		<el-table ref="multipleTable" :data="data" tooltip-effect="dark" v-loading="loading" style="width: 100%">
		  ...
		  <el-table-column prop="price" label="单价(元)"></el-table-column>
			<el-table-column prop="recommBuyCount" label="荐购次数"></el-table-column>
		</el-table>
			<!-- 高级统计 -->
...
import { recommBuyList,recommBuyExport } from "@/request/count";
...
export default {
  ...
  methods: {
    ...
    getData(data = this.form) {
      getData.call(this, recommBuyList, data);
    },
    searchData() {
      ...
      this.form = {
        ...
        pageSize: 5,
        pageNum: 1,
        prebookStatus: this.searchForm.prebookStatus
      };
    ...
    //导出多条图书荐购统计
     exportBorrowBooks(){
        this.$confirm('是否要导出图书荐购统计?', '提示', {
          ...
          recommBuyExport(JSON.stringify(obj)).then(data=>{
            if(data.data.code==0){
