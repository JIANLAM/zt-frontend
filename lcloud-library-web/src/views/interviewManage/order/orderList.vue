      ...
      <span v-if="searchButton">
 		...
 		<el-table
			...
			<el-table-column
		     ...
		     <el-table-column
		      ...
		      <template slot-scope="scope">
            <span v-for="(item,index) in power" :key="index">
						  <el-button type="text" size="mini" @click="showWuliu()" v-if="item.name.trim()=='物流信息'">物流信息</el-button>
            </span>
		...
		<dia-log :visible.sync="showDia" :title="isWuliu?'物流信息':'订单详情'">
			<div class="borders" v-if="isWuliu">
				<ul>
					<li v-for="item in wuliuData" :key='item.id' class="wuliuData">
  		...
  		getData(obj){
			...
			getSelectedBooks(obj){
  			...
  			selectedBooks(obj).then(data=>{
				...
				//导出多条订购单信息
				exportBudget(){
					this.$confirm('是否要导出这'+this.selection.length+'条订购清单?', '提示', {
