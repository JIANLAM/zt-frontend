<template>
	<div class="container">
      ...
      <span v-for="(item,index) in power" :key="index">
		...
		<el-button v-if="item.name.trim()=='新增'" type="success" icon="el-icon-plus" @click="addOrder">新增</el-button>
		<el-button v-if="item.name.trim()=='删除'" type="danger" icon="el-icon-delete" :disabled="isCheck" @click="deletSome">删除</el-button>
		<el-button v-if="item.name.trim()=='导出'" type="warning" icon="iconfont icon-daoru" :disabled="isCheck" :style="{marginBottom:'10px'}" @click="exportOrder">导出</el-button>
      </span>
    </el-form>
			...
			prop="ruleName"
		      ...
		      >
				<template slot-scope="scope">
					<span v-if="!scope.row.ruleName">关闭规则</span>
		     ...
		     <el-table-column
		      ...
		      >
		      	<template slot-scope="scope">
               <span v-for="(item,index) in power" :key="index">
                  <el-button v-if="item.name.trim()=='修改'" type="text"  @click="updateRule(scope.row)">修改</el-button>
                  <el-button v-if="item.name.trim()=='删除'" type="text"  :style="{color:'#f56c6c'}" @click="delet(scope.row)" >删除</el-button>
               </span>
			   </template>
		    </el-table-column>
		...
		<dia-log :visible.sync="visible" :title="diaLogTitle" :center="addBook" :width="addBook?'80%':'30%'">
			<div v-if="addBook">
				    ...
				    <el-tab-pane >
					    <span slot="label"><i class="el-icon-search"></i> 查询图书</span>
							...
							<el-form-item label="数据来源">
							    ...
							    size="small">
							      <el-option label="新华集团" value="1"></el-option>
							      <el-option label="本馆图书" value="2"></el-option>
							      <el-option label="豆瓣图书" value="3"></el-option>
							      <el-option label="成员馆图书" value="4"></el-option>
							    </el-select>
							</el-form-item>
			...
			<el-form :model="diaLogForm" :rules="rules" label-width="80px" ref="ruleForm" status-icon v-else>
			    ...
			    <el-select placeholder="请选择" v-model="diaLogForm.budgetId" style="width: 70%">
					<el-option :label="item.name" :value="item.id" v-for="item in budgetList" :key="item.id"></el-option>
			    </el-select>
			  </el-form-item>
			  <el-form-item label="收货地址">
			    ...
			    <el-form-item label="订购规则" prop="ruleId">
			      <el-select v-model="diaLogForm.ruleId" placeholder="请选择订购规则" style="width: 70%">
			        ...
			        <el-option :label="item.ruleName" :value="item.id.toString()" v-for="item in ruleList" :key="item.id"></el-option>
			      </el-select>
			    </el-form-item>
...
import { orderbuy,orderbuyAdd,orderbuySave,orderbuyrule,orderbuyUpdate,
	orderbuyDelete,searchBookList,addBooks,selectedBooks,setbookqty,bookDelete,dupcheck,dupsearch,orderbuyCommit,exportOrders,importFile,exportFile,autoCheck,adopt} from '@/request/order'
import DiaLog from '@/components/Dialog'
  export default{
      ...
      getPowerArr(){
	    ...
	    //添加查看图书搜索按钮事件
	    searchBook(){
	    	Object.assign(this.addForm,this.addBookForm);
	    	this.searchBookData(JSON.stringify(this.addForm));
	    },
	    //添加查看图书多选事件
	    ...
	    //批量添加图书事件
	    addBooks(){
	    	 this.$confirm('是否要添加这'+this.addBookSelection.length+'本图书?', '提示', {
		          confirmButtonText: '确定',
		          cancelButtonText: '取消',
		          type: 'warning'
		        }).then(() => {
	    	...
	    	return new Promise((resolve,reject)=>{
		    	bookDelete(obj).then(data=>{
		    		if(data.data.code==0){
		    			resolve();
		    			this.$message({
					          message:'移除'+data.data.count+'本图书成功!',
					          type: 'success'
					        });
		    		}else{
