<template>
  <div id="catalong">
    ...
    <el-select v-model="searchFrom.status" placeholder="请选择" :style="{width:'120px'}">
      <el-option label="所有" value=""></el-option>
      <el-option label="全部还回" value="0"></el-option>
      <el-option label="部分还回" value="1"></el-option>
    </el-select>
  </el-form-item>
		<el-form-item>
	...
	<el-table
		...
		<el-table-column  prop="pubdate"  label="出版时间" 	:formatter="YtdFormat"></el-table-column>
		<el-table-column  prop="recount"  label="借购总册数" >  </el-table-column>
		<el-table-column  prop="backCount"  label="已还回册数">  </el-table-column>
        ...
        <el-row>
                      ...
                      <div class="three">
                        <el-form :label-position="labelPosition" label-width="70px" :model="form" size="mini">
                            ...
                            <el-form-item label="复本数" prop="recount" :rules="[{ required: true, message: '复本数不能为空'},{ type: 'number', message: '复本数必须为数字值'}]">
                              <el-input v-model="form.recount" v-on:keyup.enter.native="addData"></el-input>
                            </el-form-item>
          ...
          <el-col :span="12" style="float:right">
            <el-card style="min-height:870px">
              <div>外部图书</div>
                <div class="external">
                  <el-form :inline="true" :model="searchBookFrom" size="medium" style="margin:20px 0">
                    <el-form-item >
                        <el-input v-model="searchBookFrom.searchValue" clearable>
                          <el-select v-model="searchBookFrom.searchKey" slot="prepend" :style="{width:'120px'}">
                              <el-option label="ISBN" value="isbn"></el-option>
                    ...
                    <el-button type="primary" @click="searchBookData" round size="medium">查询</el-button>
                    <el-checkbox-group  v-model="source">
                      <el-checkbox label="新华集团"></el-checkbox>
                      <el-checkbox label="本馆图书"></el-checkbox>
                      <el-checkbox label="豆瓣图书"></el-checkbox>
                      <el-checkbox label="成员馆图书"></el-checkbox>
                    </el-checkbox-group>
                  </el-form>
                  ...
                  <el-tabs v-model="activeName2" @tab-click="handleClick">
                    <el-tab-pane label="查询结果" name="first">
                        <el-table
                      :data="data1" border  style="width: 100%;margin-top:20px"  v-loading='loading2'>
                      <el-table-column label="序号" type="index" width="50"></el-table-column>
                       <el-table-column  prop="circulation"   label="操作">
                        <template slot-scope="scope">
                          <el-button type="text" @click="addOut(scope.row)">查看详情</el-button>
                        </template>
                      </el-table-column>
                      <el-table-column  prop="title"  label="图书名称">  </el-table-column>
                      <el-table-column  prop="isbn"  label="ISBN">  </el-table-column>
                  ...
                  @size-change="handleSizeChange1"
                    ...
                    <el-tab-pane label="书目详情" name="second">
                              ...
                              <img :src="bookForm.pic" style="display:inline-block;width:148px;height:148px">
                                    ...
                                    <el-form-item label="版次">
                                        <el-select v-model="bookForm.revision"  placeholder="请选择" :disabled="true">
                                          <el-option label="第一版" value="1"></el-option>
                                          <el-option label="第二版" value="2"></el-option>
                                          <el-option label="第三版" value="1"></el-option>
                                    ...
                                    <el-form-item label="语种">
                                        ...
                                        <el-option label="世界语" value="3"></el-option>
                                        <el-option label="法语" value="4"></el-option>
                                        <el-option label="德语" value="5"></el-option>
                                        <el-option label="日语" value="6"></el-option>
                                        <el-option label="拉丁语" value="7"></el-option>
                                        <el-option label="俄语" value="8"></el-option>
                                        <el-option label="藏语" value="9"></el-option>
                                        </el-select>
                                    </el-form-item>
                                    <el-form-item label="开本">
                                        <el-select v-model="bookForm.name"  placeholder="请选择" :disabled="true">
                                    ...
                                    <el-form-item label="载体类型">
                                        ...
                                        <el-option label="录像带" value="4"></el-option>
                                        <el-option label="光碟" value="5"></el-option>
                                        </el-select>
                                    </el-form-item>
                                    <el-form-item label="装帧类型">
                                        <el-select v-model="bookForm.binding"  placeholder="请选择" :disabled="true">
                                        <el-option label="平装" value="1"></el-option>
                            ...
                            <div class="two">
                              <el-form :label-position="labelPosition" label-width="90px" :model="bookForm" size="mini">
                                  ...
                                  <el-form-item label="获得方式">
                                    ...
                                    <el-option label="捐赠" value="2"></el-option>
                                    <el-option label="交换" value="3"></el-option>
                                    <el-option label="自编" value="4"></el-option>
                                    </el-select>
                                  </el-form-item>
                              </el-form>
                            </div>
                      ...
                      <div style="text-align:right;height:120px;line-height:150px" >
                        <el-button icon="el-icon-download" type="primary" @click="downloadData(downloadbook1)">下载到中央库</el-button>
                        <el-button icon="el-icon-download" type="success" @click="downloadData(downloadbook2)">下载到采访库</el-button>
                        <el-button icon="el-icon-warning" type="danger" @click="coverData">覆盖</el-button>
                      </div>
                    </el-tab-pane>
                  </el-tabs>
        </div>
...
import {catalogDetail,catalogSave,getmaxbarcode,foreignbooklist,foreignbookinfo,lendbuylist,downloadbook,downloadprebook} from '@/request/catalog'
...
export default {
  data() {
    return {
      ...
      addForm:{},
      collAddressList:[{label:"新华图书馆",value:"1"},{label:"深圳图书馆",value:"2"}],
      partitionList:[{label:"20180806",value:"1"}],
      circulationList:[{label:"新华图书馆",value:"1"},{label:"深圳图书馆",value:"2"}],
      out:true,
      source:[],
      bookForm:{},
  ...
  methods: {
    ...
    add(row){
      ...
      let obj={isbn:row.isbn,recount:row.recount}
